...
fix_percentage = config["rebalance_percentage"]
...
fix_value_lst = [fix_percentage]  # Rebalancing Ratio (ใส่สัดส่วนที่ต้องการ Rebalance)
...
# Rebalance Condition
min_diff_percentage = config["min_diff_percentage"]  # Minimum Rebalance Size (%)
        ...
        while len(wallet) < len(token_name_lst) + 1:
            ...
            for asset_name in token_fix_value:
                if asset_name not in asset_in_wallet:
                    ...
                    # Create BUY params
                    initial_buy = cash * fix_percentage
                    buy_size = initial_buy / price
                    buy_price = bid_price
            ...
            if cash > 0 and len(wallet) == len(token_name_lst) + 1:
                ...
                for item in wallet:
                    ...
                    if asset_name != 'USD':
                        ...
                        fixed_value = total_asset_value * fix_percentage # Fixed_value calculation based on percentage
                        min_reb_size = total_asset_value * min_diff_percentage # Minimum rebalance size based on percentage
                        ...
                        if asset_name in token_fix_value.keys():
                            ...
                            print('{} Fix Value is {}'.format(asset_name, fixed_value))
                            print('Current Diff is {}'.format(diff))
                            print('Minimum Rebalance value = {}'.format(min_reb_size))
